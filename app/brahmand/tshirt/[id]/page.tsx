...
const ProductDetailsPage = () => {
  ...
  const [sizeChartImage] = useState("/images/i1.png"); // Path to your size chart image
  const [isDescriptionOpen, setIsDescriptionOpen] = useState(false);
  const [isArtistDescriptionOpen, setIsArtistDescriptionOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        setSelectedImage(selectedProduct.image); // Set initial image

        const otherProducts = data.filter(
          (prod: Product) => prod.id !== selectedProduct.id
        );
        setOtherProducts(otherProducts.slice(0, 5));
      } catch (err) {
  ...
  {/* Wrapper for Stars, Wishlist, and Image */}
    ...
    <div className="relative">
      ...
      />
          ...
          <div className="flex space-x-20">
            {(() => {
              const photosToDisplay = product.stockPhotos.filter(
                (photo) => photo !== selectedImage
              );

              while (photosToDisplay.length < 3) {
                photosToDisplay.push(product.image);
              }

              return photosToDisplay.slice(0, 3).map((photo, index) => (
                <div key={index} className="relative">
                  <Image
                    ...
                    onClick={() => setSelectedImage(photo)} // Set selected image
                  />
                </div>
              ));
            })()}
  ...
  <h2 className="text-3xl font-bold text-gray-200 mb-4">Other&apos;s Also Bought</h2>
  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-10">
    {otherProducts.map((product) => (
      <Link href={`/brahmand/tshirt/${product.id}`} key={product.id} passHref>
        <div className="p-4 rounded-lg text-center flex flex-col items-center bg-transparent hover:shadow-lg transition cursor-pointer">
          <Image
            src={product.image}
            alt={product.name}
            width={200}
